...
public class Vector4f extends Renderable implements Serializable {
	...
	public Vector4f(Vector3f xyz, float w) {
		this.points[0] = xyz.points[0];
		this.points[1] = xyz.points[1];
		this.points[2] = xyz.points[2];
		this.points[3] = w;
	}
	...
	public void copyFromV3f(Vector3f input, float w) {
		points[0] = (input.points[0]);
		points[1] = (input.points[1]);
		points[2] = (input.points[2]);
		points[3] = (w);
	}
