...
public class Vector3f extends Renderable {
	...
	// an array
	// at run time can cause slowness so instead we use one array and extract the individual variables with get methods.
	...
	public Vector3f(Vector4f vector) {
		if (vector.w() != 0) {
			this.points[0] = vector.points[0] / vector.points[3];
			this.points[1] = vector.points[1] / vector.points[3];
			this.points[2] = vector.points[2] / vector.points[3];
		} else {
			this.points[0] = vector.points[0];
			this.points[1] = vector.points[1];
			this.points[2] = vector.points[2];
	...
	public void add(float addition) {
		this.points[0] += addition;
		this.points[1] += addition;
		this.points[2] += addition;
	}
